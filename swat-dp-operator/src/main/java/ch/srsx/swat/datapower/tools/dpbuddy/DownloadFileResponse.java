...
public class DownloadFileResponse extends DPResponse {
	...
	public void saveFile(String file) {
		try {
			...
			Base64OutputStream fos = new Base64OutputStream(new BufferedOutputStream(new FileOutputStream(f)), false);

			writeFileContent(fos);
			fos.close();
		} catch (IOException e) {
			throw new DPException(String.format("Error saving file to '%s'. Caused by:\n%s", new Object[] { file, e.getMessage() }), e, new Object[0]);
		}
	}
	...
	public void saveFile(File f) {
		try {
			...
			throw new DPException(String.format("Error saving file to '%s'. Caused by:\n%s", new Object[] { f.getAbsoluteFile(), e.getMessage() }), e, new Object[0]);
		}
	}
	...
	public void writeFileContent(OutputStream outStream) throws IOException {
		...
		if (files.size() > 1) {
			throw new DPException("Detected multiple file entries in the response. This is not allowed by the schema. XML of the response:\n" + this, new Object[0]);
		}
