...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:xalan="http://xml.apache.org/xslt"
   ...
   <xsl:template match="//file">
      ...
      <xsl:variable name="request-filename" select="concat('get-file-', $index,'-',translate(translate($filename,':',''),'/','_'),'.xml')" />
      <xsl:variable name="response-filename" select="concat('get-file-', $index,'-',translate(translate($filename,':',''),'/','_'),'-response.xml')" />
      <xsl:message>
