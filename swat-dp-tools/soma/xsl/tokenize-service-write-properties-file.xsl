...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt"
    ...
    <xsl:template match="//XMLFirewallService">
        ...
        <xsl:value-of select="concat('# XMLFW ', $label, '&#xa;')" />
        <xsl:for-each select="*">
            <xsl:choose>
                <xsl:when test="local-name(.)='mAdminState'">
                    <xsl:call-template name="print-token">
                ...
                <xsl:otherwise>
                    <xsl:message>
                        <xsl:value-of select="'nothing to do'" />
                    </xsl:message>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
    ...
    <xsl:template match="//MultiProtocolGateway">
        ...
        <xsl:call-template name="print-attribute-token">
        ...
        <xsl:for-each select="*">
            <xsl:choose>
                ...
                <xsl:when test="local-name(.)='FrontProtocol'">
                    <xsl:call-template name="print-attribute-and-value-token">
    ...
    <xsl:template name="print-attribute-token">
    ...
    <xsl:template name="print-attribute-and-value-token">
