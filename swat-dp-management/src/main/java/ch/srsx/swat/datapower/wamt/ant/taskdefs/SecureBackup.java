 ...
 * 
 * The SecureBackup ANT task uses the WebSphere Appliance Management Toolkit
 * (WAMT) to generate and download a secure backup of a DataPower device.
 * 
 * Note: This task only works if secure backup mode has been enabled during
...
public class SecureBackup extends Task {
	...
	static {
		final InputStream inputStream = SecureBackup.class.getClassLoader()
	...
	public void execute() throws BuildException {
		...
		String targetDir = targetBaseDir + separator + dateSubdir + separator
				+ symbolicName;
		...
		if (!result && !target.exists()) {
			throw new BuildException("Could not create target direcotory '"
					+ targetDir + "'");
		} else if (!forceOverwrite && target.list().length > 0) {
			throw new BuildException(
					"The target direcotory '"
							+ targetDir
		...
		try {
			progressContainer = device.backup(certname, null, new URI(
					"file:///" + targetDir), includeISCSI, includeRAID);
			progressContainer.blockAndTrace(Level.FINER);
			if (progressContainer.hasError()) {
			...
			throw new BuildException(
					"Could not execute backup method on device "
