...
import ch.srsx.swat.datapower.dpbuddy.FilestoreRequest;
import ch.srsx.swat.datapower.dpbuddy.FilestoreResponse;
 ...
 * 
 * The FileManagement ANT task is an extension for the Myarch dpbuddy that
 * facilitates the download of files from an IBM WebSphere DataPower SOA
 * appliance.
 * 
 * Note: This task depends on SWAT specific extension classes for the Myarch
 * dpbuddy framework.
 * 
 * {@link DownloadFileRequest} {@link DownloadFileResponse}
 * {@link FilestoreRequest} {@link FilestoreResponse}
 * 
 * @author <a href="mailto:pshah@schlagundrahm.ch">Pierce Shah</a>
 * 
 */
public class FileManagement extends BaseDPTask {
	...
	protected void executeDPTask() {
		...
		if (fileName == null || fileName.length() == 0 || regex) {
			if (listFiles) {
				logger.info("getting file store information for location '"
						+ location + "'");
			} else {
				if (regex) {
					logger.info("download files from location '" + location
							+ DP_SEPARATOR + subDir
							+ "' using regular expression '" + fileName + "'");
					pattern = Pattern.compile(fileName);
				} else {
					logger.info("download all files from location '" + location
							+ DP_SEPARATOR + subDir + "'");
				}
			}
			...
			if (listFiles) {
				...
				((FilestoreResponse) response).writeResponseFile(targetDir
						+ FILE_SEPARATOR + getHostName()
						+ "-filestore-info.xml");
			} else {
				for (String file : ((FilestoreResponse) response)
					...
					File toFile = getOutpuFile(getMapper(),
							file.substring(file.indexOf(":") + 2));
					if (!overwrite && toFile.exists()) {
						handleError("Target file '"
								+ toFile
								+ "' already exists! Use overwrite mode if needed.");
					}
			...
			logger.info("download file '" + fileName + "' from location '"
					+ location + subDir + "'");
			if ((targetFile == null) || (targetFile.length() == 0)) {
				targetFile = fileName;
			}
			...
			if (!overwrite && toFile.exists()) {
				handleError("Target file '" + toFile
						+ "' already exists! Use overwrite mode if needed.");
			}
			DownloadFileRequest fileRequest = new DownloadFileRequest();
			fileRequest.setFromFile(location + "///" + subDir + DP_SEPARATOR
					+ fileName);
			response = (DownloadFileResponse) executeRequest(fileRequest);
			((DownloadFileResponse) response).saveFile(targetDir
					+ FILE_SEPARATOR + targetFile);
		}
	...
	private String getHostName() {
		...
		String url = getProject()
				.getProperty(envPrefix + "." + urlPropertyName);
	...
	private void logFilestoreResponse(FilestoreResponse filestoreResponse) {
		...
		for (Element file : filestoreResponse.getTopLevelFiles()) {
			logger.info("\t" + file.getAttributeValue("name") + "\t("
					+ file.getChildText("modified") + ")");
		}
	...
	private File getOutpuFile(FileNameMapper mapper, String fileName) {
		...
		if ((outFileName == null) || (outFileName.length == 0)) {
			log("Skipping " + this.fileName
					+ " it cannot get mapped to output.", 3);
